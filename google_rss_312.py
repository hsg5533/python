        ...
        for file_path in self.file_paths:
            ...
            if text:
                ...
                if sentences:
                    ...
                    # 각 문장과 임베딩 출력 (임베딩 벡터의 앞 5개 값만 표시)
...
class MainWindow(QMainWindow):
    ...
    def fetch_news(self):
        ...
        이후, 출판 시간이 전날 21시부터 다음 날 8시 59분 59초 사이인 뉴스만 리스트에 추가하고,
        상위 결과를 텔레그램으로 전송합니다.
        """
        ...
        for entry in feed.entries:
            # published_parsed가 존재하면 시간 정보를 가져옴
            if hasattr(entry, 'published_parsed'):
                pub = entry.published_parsed
                # 전날 21시 ~ 다음 날 8시 59분 59초 조건:
                # tm_hour가 21,22,23이면 포함되고, 0 ~ 8시도 포함
                if not (pub.tm_hour >= 21 or pub.tm_hour < 9):
                    continue  # 해당 시간대가 아니면 건너뜁니다.
            else:
                continue  # published 정보가 없으면 건너뜁니다.
